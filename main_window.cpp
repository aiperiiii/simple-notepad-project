...
main_window::main_window() {
    ...
    highlighter = new spell_checker_highlighter(editor->document(), checker.get());
...
void main_window::update_status_bar() {
    ...
    status_cursor->setText(QString("Ln: %1, Col: %2").arg(ln).arg(col));
}
...
void main_window::show_word_frequency() {
    ...
    notepad_sort::sort_by_frequency(sorted_freq);
...
void main_window::show_context_menu(const QPoint &pos) {
    auto word_cursor = editor->cursorForPosition(pos);
    word_cursor.select(QTextCursor::WordUnderCursor);
    const QString word = word_cursor.selectedText();

    QMenu *menu = editor->createStandardContextMenu();

    if (!word.isEmpty() && checker && !checker->is_correct(word.toStdString())) {
        menu->addSeparator();
        ...
        if (suggestions.empty()) {
            ...
            for (const auto &suggestion: suggestions) {
                const QString sug = QString::fromStdString(suggestion);
                connect(menu->addAction(sug), &QAction::triggered, this, [this, word_cursor, sug]() mutable {
                    word_cursor.insertText(sug);
                });
            }
        }
    }
